...
function buildClient() {
  ...
  return (new Promise((resolve, reject) => {
    webpack({
      entry: path.join(srcBasePath, 'watcher.js'),
      target: 'node',
    ...
    let cliBuildList = [
      {
        arch: 'linux-x64',
        output: 'graffiti-daemon-amd64',
        target: 'linux-x64-14.15.3'
      },
      {
        arch: 'linux-x86',
        output: 'graffiti-daemon-x86',
        target: 'linux-x86-14.15.3'
      },
      {
        arch: 'win32-x64',
        output: 'graffiti-daemon-win64.exe',
        target: 'windows-x64-14.15.3'
      },
      {
        arch: 'win32-x86',
        output: 'graffiti-daemon-win32.exe',
